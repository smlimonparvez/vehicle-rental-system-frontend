      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8">
          ...
          <div className="flex gap-2">
            {DEMO.map(({ label, email, password, cls }) => (
              <button key={label} type="button" onClick={() => fillDemo(email, password)}
                className={`flex-1 border rounded-xl px-3 py-2.5 text-left transition-colors ${cls}`}>
        ...
        <div className="relative mb-5">
          ...
          <div className="relative flex justify-center">
            <span className="bg-white dark:bg-gray-800 px-3 text-xs text-gray-400">or sign in manually</span>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-blue-600 text-white py-2.5 rounded-lg font-semibold hover:bg-blue-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed mt-1">
