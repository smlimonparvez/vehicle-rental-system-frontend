...
function VehicleImage({ vehicle }: { vehicle: Vehicle }) {
  ...
  return (
    <div className={`h-44 flex items-center justify-center relative overflow-hidden ${showFallback ? `bg-gradient-to-br ${vehicleGradient(vehicle.type)}` : 'bg-gray-100 dark:bg-gray-700'}`}>
      {!showFallback ? (
      ...
      <div className="absolute top-3 right-3">
        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${vehicle.availability_status === 'available' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {vehicle.availability_status === 'available' ? '✓ Available' : '✗ Booked'}
        </span>
      </div>
...
export default function FeaturedVehicles() {
  ...
  useEffect(() => {
    getVehiclesApi().then(r => {
      if (r.success) { setVehicles(r.data || []); setFiltered(r.data || []); }
    ...
    <section ref={ref} className="py-24 bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className={`text-center mb-12 transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="text-4xl font-bold text-gray-900 dark:text-white mt-2 mb-4">Choose Your Perfect Ride</h2>
        ...
        <div className={`flex justify-center gap-2 flex-wrap mb-10 transition-all duration-700 delay-200 ${visible ? 'opacity-100' : 'opacity-0'}`}>
          {TYPES.map(t => (
            <button key={t} onClick={() => setActiveType(t)}
              className={`px-5 py-2 rounded-full text-sm font-medium capitalize transition-all ${activeType === t ? 'bg-blue-600 text-white shadow-lg shadow-blue-200 dark:shadow-blue-900' : 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-blue-300'}`}>
        ...
        <div className={`text-center mt-12 transition-all duration-700 delay-500 ${visible ? 'opacity-100' : 'opacity-0'}`}>
          <Link href="/vehicles" className="inline-flex items-center gap-2 border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white px-8 py-3 rounded-xl font-semibold transition-all group">
            View All Vehicles <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </div>
    </section>
